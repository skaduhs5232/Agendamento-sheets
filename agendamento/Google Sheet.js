...
function obterHorarios(scriptURL) {
  fetch(scriptURL)
    .then((response) => response.json())
    .then((data) => {
      const horariosOcupados = data.hora; // Horários ocupados recebidos da planilha
      ...
      const horariosFiltrados = horariosDisponiveis.filter(hora => !horariosOcupados.includes(hora));
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    selectProfissional.addEventListener("change", (event) => {
      ...
      if (profissionalSelecionado) {
        ...
        obterHorarios(scriptURL);
        console.log(`URL do profissional selecionado: ${scriptURL}`);
        obterHorarios(scriptURL); // Chama a função para obter horários
      } else {
